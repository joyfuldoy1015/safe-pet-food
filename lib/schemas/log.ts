...
const reviewLogBaseSchema = z.object({
  pet_id: z.string().uuid('반려동물 ID는 유효한 UUID여야 합니다'),
  category: z.enum(['feed', 'snack', 'supplement', 'toilet'], {
    errorMap: () => ({ message: '제품군은 사료, 간식, 영양제, 화장실 중 하나여야 합니다' })
  }),
...
export const reviewLogCreateSchema = reviewLogBaseSchema.extend({
...
export const reviewLogUpdateSchema = reviewLogBaseSchema.partial().extend({
